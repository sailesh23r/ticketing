...
export default function SharedReportPage(props: { params: Promise<{ token: string }> | { token: string } }) {
  // Support both direct object and promised params (future Next.js behavior)
  // @ts-expect-error experimental unwrap with React.use()
  const resolvedParams = typeof props.params.then === 'function' ? React.use(props.params) : props.params;
  const token = (resolvedParams as { token: string }).token;
  ...
  return (
    <div ref={rootRef} className="mx-auto max-w-[1600px] p-6 flex flex-col gap-6">
                ...
                <table className="w-full text-xs table-fixed border-collapse [&_th]:text-[10px] [&_th]:uppercase [&_th]:tracking-wide" aria-label="Shared ticket report table">
                  ...
                  <tbody className="[&_tr]:transition-colors">
                              ...
                              <td colSpan={10} className="bg-muted/20 border border-gray-100 p-3">
                                <div className="grid gap-3 md:grid-cols-4 text-xs">
                                  <div>
                                    ...
                                    <div className="font-medium">{/* not included in shared payload */}—</div>
                                  </div>
                                  <div>
                                    <div className="text-muted-foreground">Last Updated</div>
                                    <div className="font-medium">{r.updatedAt ? formatDate(r.updatedAt) : '—'}</div>
                                  </div>
                                  <div>
                                  ...
                                  <div>
                                    ...
                                    <div className="font-medium">{(r.status === 'resolved' || r.status === 'closed') && r.updatedAt ? formatDate(r.updatedAt) : '—'}</div>
                                  </div>
                                  <div>
                                    <div className="text-muted-foreground">Resolved By (Agent)</div>
                                    <div className="font-medium">{r.completedByUserName || r.completedByUserId || '—'}</div>
                                  </div>
                                  <div>
                    ...
                    {report.rows.length === 0 && (
                      <tr>
                        <td colSpan={10} className="text-center py-8 text-xs text-muted-foreground border border-gray-100">No tickets.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
      ...
      {/* KPI Cards & Charts (render even if expired? we hide on expired) */}
      {!expired && report && (
        <>
          <SectionCardsNew project={params?.project} hideControls paddings="px-4 lg:px-2" />
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 items-stretch">
            <ChartAreaInteractive team={params?.team} project={params?.project} days={params?.days ?? undefined} start={params?.start} end={params?.end} hideControls />
            <ChartLineInteractive team={params?.team} project={params?.project} days={params?.days ?? undefined} start={params?.start} end={params?.end} hideControls />
            <div className="flex flex-col gap-4">
              <ChartPieDonutText team={params?.team} project={params?.project} />
              <TopPerformers team={params?.team} project={params?.project} />
            </div>
          </div>
        </>
      )}
