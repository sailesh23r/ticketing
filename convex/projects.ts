...
export const createProject = mutation({
  args: { slug: v.string(), name: v.string(), description: v.optional(v.string()), members: v.optional(v.array(v.string())) },
  handler: async (ctx, args) => {
...
export const updateProject = mutation({
  args: { slug: v.string(), name: v.optional(v.string()), description: v.optional(v.string()), members: v.optional(v.array(v.string())) },
  handler: async (ctx, args) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) throw new Error("Not authenticated");
    const roles = await userRoles(ctx, identity.subject);
